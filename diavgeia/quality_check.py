...
def calculate_word_count(text: str) -> int:
    ...
    words = re.findall(r"[A-Za-zΑ-Ωα-ωΆ-Ώά-ώϊϋΐΰ]{2,}", text)

    return len(words)


def calculate_corrupted_accent_count(text: str) -> int:
    """
    ...
    return len(
        re.findall(r"[Α-Ωα-ωάέήίόύώϊϋΐΰ][\u0300-\u036f]", text))
...
def evaluate_text_quality(text: str,) -> dict[str, Any]:
    ...
    letter_ratio = calculate_letter_ratio(text)

    word_count = calculate_word_count(text)

    corrupted_accent_count = (
        calculate_corrupted_accent_count(text))
    ...
    if corrupted_accent_count >= 10:
        reasons.append("suspicious_combining_accents")

    if symbol_ratio > 0.25:
        reasons.append("high_symbol_ratio")

    status = ("suspicious" if reasons else "good")
...
def run_quality_check() -> None:
    ...
    QUALITY_REPORT_FILE.unlink(missing_ok=True)
    ...
    for record in records:
        ...
        report = {"ada": record.get("ada"), **quality}

        append_jsonl(QUALITY_REPORT_FILE, report)

        if (quality["status"] == "suspicious"):

            suspicious_count += 1

            append_jsonl(SUSPICIOUS_FILE, report)
    ...
    print(f"Good documents: {good_count}")
    ...
    print(f"\nQuality report: {QUALITY_REPORT_FILE.resolve()}")

    print(f"Suspicious documents: {SUSPICIOUS_FILE.resolve()}")
